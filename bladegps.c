...
/*
 ...
 =======================================================================================================================
 */
static void* stdin_quit_task(void* arg)
{
  int c;
...
size_t fifo_read(int16_t* buffer, size_t samples, sim_t* s)
{
  ...
  size_t length = get_sample_length(s);
...
{
  int status = pthread_create(&(s->tx.thread), NULL, tx_task, s);
...
{
  int status = pthread_create(&(s->gps.thread), NULL, gps_task, s);
...
void usage(void)
{
  printf(
    ...
    ((double)USER_MOTION_SIZE) / 10.0, TX_GAIN, NORTH_KEY, SOUTH_KEY, EAST_KEY, WEST_KEY);
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    ...
    if (tx_gain < min_gain)
      tx_gain = (int)min_gain;
    else if (tx_gain > max_gain)
      tx_gain = (int)max_gain;
  }
  ...
  s.status = bladerf_sync_config(s.tx.dev, tx_channel, BLADERF_FORMAT_SC16_Q11, NUM_BUFFERS, SAMPLES_PER_BUFFER, NUM_TRANSFERS, TIMEOUT_MS);
